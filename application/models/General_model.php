...
class General_model extends CI_Model
{
    ...
    public function get_all($table,$fields="",$where="")
    {
        ...
        $array['resultQuery'] = $this->db->last_query();        
        return $array;
    }
        ...
        if(isset($data) && !empty($data) && is_array($data))
        {
            ...
            if(isset($data['where']) && !empty($data['where']) && is_array($data['where']))
            {
                foreach($data['where'] as $where)
                {
                    ...
                    if(isset($where['custom'],$where['custom_where']) && !empty($where['custom']) && !empty($where['custom_where']))
					{
                        $this->db->where($where['custom_where'], null, false);
                    }
                }
            }
    ...
    //temporery
    public function get_ads($page_name,$page_type){
        $this->db->select('*');
        $this->db->from('bots_ads as ad');
        $this->db->where('ad.page_name',$page_name);
        $this->db->where('ad.page_type',$page_type);
        $this->db->where('ad.status',1);
        $this->db->where('ad.start_date <=', date('Y-m-d'));
        $this->db->where('ad.end_date >=', date('Y-m-d '));
        $query = $this->db->get();
