...
class Product_model_1 extends CI_Model
{
 ...
 public function fetch_query($manufactured, $product_type, $product_years, $product_model)
 {
    ...
    INNER JOIN tb_manufactured as b ON b.manufactured_id = p.manufactured INNER JOIN tb_years as c ON c.year_id = p.year INNER JOIN tb_product_type as d ON d.product_type_id = p.product_type INNER JOIN tb_models as e ON e.model_id = p.model WHERE p.status = '1'";
    if(isset($manufactured))
    {
       
      if($manufactured !="")
       {
        $product_man_filter =  $manufactured;
        $query .= ' AND p.manufactured ='.$product_man_filter;
       }
    }
    ...
    if(isset($product_years))
    {
       if($product_years !="")
       {
        $product_years_filter =  $product_years;
        $query .= ' AND p.year ='.$product_years_filter;
       }
    ...
    if(isset($product_model))
    {
       if($product_model !="")
       {
        ...
        $query .= ' AND p.model ='.$product_model;
       }
    }
    // echo $query;
    return $query;
